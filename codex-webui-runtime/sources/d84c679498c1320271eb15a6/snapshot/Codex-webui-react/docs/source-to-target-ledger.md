...
| `public/index.html`, `public/js/app.js`, `public/css/app.css` topbar question/reply jump controls | `src/client/legacy-shell.ts`, `static/js/app.js`, `public/js/app.js`, `static/css/app.css`, `public/css/app.css` | verified-equivalent | Shell exposes `questionJumpUp`/`questionJumpDown`; copied JS scrolls to loaded `.bubble.user` and `.bubble.agent` anchors; copied CSS keeps the control compact |
