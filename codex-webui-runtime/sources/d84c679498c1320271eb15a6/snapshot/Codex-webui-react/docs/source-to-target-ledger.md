...
| `src/server.ts` recycle restore scan, summary extraction, keyword filtering, archive-time listing, selectable day window, and `/session/restore` | `server/server.ts` | verified-equivalent | Ported helpers and routes; default history project is `历史对话`; candidates filter by recycle archive mtime, selected `days`, and optional query; parity checks route/helper declarations |
...
| `src/server.ts` project folder create/rename routes | `server/server.ts` | verified-equivalent | Copied folder name validation, child-path guard, `/project/create`, `/project/rename`, history root rewrite, and current-workdir rebase logic |
...
| `public/index.html`, `public/js/app.js`, `public/css/app.css` composer question/reply jump controls | `src/client/legacy-shell.ts`, `static/js/app.js`, `public/js/app.js`, `static/css/app.css`, `public/css/app.css` | verified-equivalent | Shell exposes `questionJumpUp`/`questionJumpDown` in `.composer-status-row`; copied JS scrolls to loaded `.bubble.user` anchors and completed assistant conclusion/end targets; copied CSS keeps the control compact |
| `public/js/app.js` local path quick preview and directory entries | `static/js/app.js`, `public/js/app.js`, `static/css/app.css`, `public/css/app.css`, `server/services/preview.ts`, `server/server.ts` | verified-equivalent | Local path links call copied preview panel logic; directory hrefs carry `data-kind="directory"` and go through `/path/open`; server accepts `file://` directory URLs and opens Windows paths through Explorer |
| `public/index.html`, `public/js/app.js`, `public/css/app.css` recycle restore dialog | `src/client/legacy-shell.ts`, `static/js/app.js`, `public/js/app.js`, `static/css/app.css`, `public/css/app.css` | verified-equivalent | Shell exposes `restoreHistoryBtn`; JS reads `/session/recycle-candidates` with selected `days`, `limit=200`, and optional `q`, calls `/session/restore`, and copied styles include filter and range controls |
| `public/js/app.js` session recycle-delete optimistic sidebar removal | `static/js/app.js`, `public/js/app.js` | verified-equivalent | Copied source logic hides archived sessions from `sessionsCache`, project entries, and SSE resume updates before `DELETE /session` returns; failure reloads sessions/projects |
| `public/index.html`, `public/js/app.js`, `public/css/app.css` project create/rename and project-thread overflow controls | `src/client/legacy-shell.ts`, `static/js/app.js`, `public/js/app.js`, `static/css/app.css`, `public/css/app.css` | verified-equivalent | Shell exposes `createProjectBtn`; JS calls `/project/create` and `/project/rename`, wires `rename-project`, and keeps running/current/recent project threads visible when collapsed |
